...
- **`CmdRepair` is gated to `DurableObject`** (`isinstance(target, DurableObject)`, ~line 88), so tools *and* garments are repairable — the Component D wear→repair convergence (D.4). Materials are data-driven: `func` reads `target.db.repair_materials or REPAIR_MATERIALS` (garments fall through to the cloth/twine default; `stone_knife` → stick+fibre, `bone_needle` → bone). Known gap: `_tool_modifier` still looks only for a `needle`, so it's garment-centric for now (backlog — generalise with Component G).
...
- **The craft quality scale is discrete; max is 110.** `_quality_for` yields only `{25 fumble, 50 failure, 100 success, 100+crit_score critical}`. Post-A-flip no positive modifier survives (tool present = 0 baseline), so max `target` = skill-cap 100 → `crit_score = 10` → **max quality = 110** (the Decomposition's earlier "112" assumed the old +20). Hence `superior = quality > 100` *is* the critical tier — with one deliberate edge: a critical at modified skill 0–9 has `crit_score = 0` → quality exactly 100 → bands `serviceable`. That is capability-banding (the item's capability equals a success), not a bug.
