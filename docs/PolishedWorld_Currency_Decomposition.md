...
> **Rev 7 · 2026-08-03** — **Component E CLOSED** (E.1 `2c75877`, E.2 `8d189f5`; 316 tests green across `tests/`, 60 of them in `tests/test_barter_currency.py`, two-session in-game protocol passed). Five deviations across the two tasks, recorded on the task entries above; none touches a locked decision, and three of them are consequences of the spec rather than departures from it — allowing coin-only offers is what breaks upstream's `decline`, and owning the `status` *key* turned out not to mean owning the `status` *class*. **The two architectural choices worth carrying forward:** re-validation measures **gross** per side while settlement executes **net**, because "did you back what you promised" and "how little needs to move" are different questions and a single number cannot answer both; and `_finish_and_clear()` computes its completion test from the *same expression* upstream uses for `fin`, which makes "coin moves if and only if goods move" structural instead of a rule someone has to remember. ⚠️ **Two lessons.** First, **mutation testing earned its keep**: disabling the staleness guard failed four tests as designed, but two survived — and the reason they survived was that the `transfer_to`-failure branch, documented as *unreachable*, fired and held the line. The comment now says what was observed rather than what was assumed. Second, **this document referred to a commit that does not exist**: Rev 6 and §6/D.1 both cited `4eacc13` for D.1, a SHA that was rewritten before the push. Both are corrected to `a635e82`. A SHA in prose is a claim like any other and decays the same way. Also recorded: Evennia Reference **Rev 20** (§7 was not merely stale but actively wrong), `world/README_barter.md` rewritten, BACKLOG **Rev 17**, and `PolishedWorld_GameGold_Economy.md` **Rev 6**, which finally replaces the superseded `CurrencyHandler` sketch. Remaining doc debt: Economic Philosophy, Source/Sink Ledger, roadmap and docs/README, all due in Component F.3.

> **Rev 6 · 2026-08-02** — **Component D CLOSED** (D.1 `a635e82`; 256 tests green across `tests/`, 72 of them new, in-game walkthrough passed). Three implementation deviations, all flagged, none touching a locked decision. **(1) `typeclasses/characters.py` was edited by a `commands/` component.** `at_pre_move` gained a branch that clears an in-progress chore and says so. The location re-check inside the payout callback already made this *correct* without the edit; what the edit buys is the **message**, and the house precedent is unambiguous — `rest` interrupts on movement in the same hook for the same reason. Twenty seconds of silence followed by an unexplained non-payment reads as a broken command. **(2) The payout is a module-level function, `_finish_task()`, not a closure or a bound method of the Command.** A Command instance is per-invocation, and keeping one alive purely to hold a callback is more object lifetime than the job needs; more usefully, a module-level function is directly callable from a test, which is the difference between testing the payout and testing a reactor. The tests read the marker off `char.ndb.working` rather than fabricating one — a fabricated marker would pass even with the identity check deleted. **(3) `_format_wait()` was added.** `forage` prints a bare `{left}s`, readable at sixty seconds and useless at two hours; "6821s" is a number, not an answer. **Decisions locked and implemented this session:** the task table lives in `commands/work_commands.py::TEMPLE_TASKS` on the house precedent (`FORAGE_COOLDOWN` and siblings all sit in their command module) and carries flavour for actor and room, at start and finish, because the house keeps flavour with the *data*; `work` with no arguments shows the **reward** for each chore, on the reasoning that a temple which pays for work but will not say what it pays is a puzzle rather than a temple, that a fixed rate protects nothing by being hidden, and that a visible low number does the design's own "this is a supplement, not a career" work that an invisible one cannot; the notice board deliberately does **not** show whether the coffers can cover a chore, since dryness is discovered by trying rather than by a board that changes under the player's feet; and the chore **takes real time** via `utils.delay`. **The delay is the substantive risk of this component and is handled rather than avoided.** S4-R1 is honoured by putting the *entire* money sequence inside the callback — nothing about the Treasury's balance is read before the wait, because a check before and a commit after would be exactly the forbidden shape with a `duration`-wide window. Five hazards are guarded: `ndb.working` blocks queueing; `at_pre_move` cancels on movement; the location re-check backstops teleport and death; `has_account` refuses payout to a logged-out body under statue-logout; and a **per-attempt identity marker** stops a stale callback from paying a later chore with the same key — the fifth was not in the original hazard list and only appears once delay is chosen. Every early return leaves the cooldown **unset**, per the locked all-or-nothing rule. Also recorded: Evennia Reference **Rev 19** §11.27 for the reusable marker pattern, BACKLOG **Rev 16** for three non-defect entries, and `PolishedWorld_GameGold_Economy.md` **Rev 5**, which marks the faucet sketch superseded and names the three-versus-five chore discrepancy the prose table always had. ⚠️ **A lesson worth carrying:** the first draft of the S4-1 regression guard grepped the module's source text for `currency.add(` and failed instantly against **its own docstring**, which explains the prohibition by quoting the forbidden call. A text search cannot tell a warning from a violation; the shipped guard walks the module's `ast` and sees only executable code. Remaining doc debt unchanged and still due at Component F.

> **Rev 5 · 2026-08-02** — **Component C CLOSED** (C.1 + C.2; 184 tests green across `tests/`, full in-game walkthrough passed including the two-character permission test). Three implementation deviations, all flagged, none touching a locked decision. **(1) `resolve_treasury()` exists alongside `get_treasury()`.** §6/C.1 specifies only `get_treasury() -> obj | None`, and that contract is untouched — `world/economy_log.py::_read_treasury` imports exactly it. But a single `None` collapses three misconfigurations that need three different fixes (unset key, dangling dbref, dbref pointing at the wrong typeclass), and the third is dangerous rather than merely unhelpful: a mistyped dbref landing on a Character resolves cleanly and *has a working `currency` handler*, so without a typeclass check `@economy mint` would create money directly in a player's purse and nothing downstream would object. `resolve_treasury()` returns a problem code, the command turns it into a sentence, and the mint is refused rather than redirected. **(2) `@economy burn` is implemented**, Treasury-only, though §6/C lists only `audit` and `mint`. The reserve-obligation figure can otherwise only ever rise, so the decrement path would ship never having run outside a unit test until Stage 8; and the Stage 4 exchange-back is a manual admin procedure, which without a subcommand means destroying money through `@py` — outside the audited surface, which is the exact hole the audit exists to close. A `from <target>` form is deliberately **not** provided: burning out of a player's wallet is confiscation, a moderation tool needing its own design conversation. **(3) `format_gamegold()` was added to `world/currency.py`**, not to the command layer, because it is denomination rendering and belongs beside `format_copper` — uncoloured per D2, unit-testable without a database. The exchange rate is 1 GameGold : 1 **Gold**, the one place in the system where an error is four orders of magnitude, so `GAMEGOLD_DECIMALS` is derived from `COPPER_PER_GOLD` and a test fails loudly if the denominations are ever tuned to a non-power-of-ten ratio. **Decisions locked this session:** mint confirmation is **argument-based** (`@economy mint 100 gold confirm`), with the bare form a **dry run** that prints the resulting reserve obligation before the obligation exists — the protection is the preview, not the word; an `ndb` two-step and a `yield` were both considered and rejected, the latter keeping the codebase's no-yield property intact. The settings key is **`TREASURY_DBREF`**, shipped **unset** with a commented placeholder, resolved **uncached** (a cached `None` from before the Treasury was built would outlive the fix and make an admin think their configuration had not taken). The reserve obligation is shown on **every** `@economy` view and is **derived** from `net_issued()`, never stored. Also recorded: Evennia Reference **Rev 18** §11.26 and `AGENTS.md` **Rev 4** for the session's most dangerous finding — `.call()` does not run command locks at all, so a permission test written with it passes **vacuously**. `PolishedWorld_GameGold_Economy.md` **Rev 4** corrects three code sketches that still showed a minting faucet, found while reading it as Component D's task-table source. Remaining doc debt unchanged and still due at Component F, plus the `actor`-field and audit-display items now in BACKLOG **Rev 15**.

> **Rev 4 · 2026-07-30** — **Component B CLOSED** (B.1 `f90e3fb`, B.2 `feb737c`; 129 tests green across `tests/`, in-game two-session test passed including a death between payments, which incidentally verifies S4-3). Two implementation deviations, both flagged, neither touching a locked decision. **(1) `can_afford()` is not called anywhere in the command layer.** §6/B.2 sketched "check affordability, then a single `transfer_to`" — but `transfer_to()` already performs the read, the sufficiency check, the debit and the credit as one unbroken block and returns `False` having touched nothing. A prior `can_afford()` would be a second read of the same fact and, worse, would write out the check-here-commit-there *shape* that S4-R1 exists to forbid, leaving a gap ready for the next editor who wants somewhere to put a prompt. Reading the balance *after* a returned `False`, to say "you only have X", is safe and is what the command does: reading to phrase is not reading to decide. **(2) The amount is parsed before the target is resolved**, the reverse of §6/B.2's ordering, and for the reason §6/B.2 itself cites — validate-then-commit (Evennia Reference §11.9). `caller.search()` has a *side effect*: it messages the player on a miss or a multimatch. Pure validation belongs ahead of the first side-effecting call, or a player who mistypes both halves is told about a target they were never going to reach instead of about the amount that could never have worked. **Locked in this session, recorded here because it is a design rule and not a deferral: `pay` is same-room permanently.** Coin is carried on the body and, from Stage 5, stays with the corpse to be looted, so carrying a large sum is a risk the player chooses; a global `pay` would cancel that risk for free. The sanctioned answer to "I don't want to carry this" is a bank or post office — a *place*, reached by a journey that can go wrong — logged in BACKLOG **Rev 14** as the replacement feature. Remote payment is therefore **rejected**, not deferred, so that no future session reads "deferred" and builds it. Also recorded: Evennia Reference **Rev 17** §11.24–11.25 and `AGENTS.md` **Rev 3** for the three reusable lessons (the silent `char2` puppet-lock failure, `Object.search()`'s candidate set, and the `.call()` separator correction — `|`, not `||`). §8 sharpened: the `add()` single-caller invariant is about **production** code; tests fund wallets through `add()` directly and always did. Remaining doc debt narrowed accordingly and still due at Component F.
...
- **Status:** ✅ **CLOSED** (`a635e82`, 2026-08-02). Shipped as
  `commands/work_commands.py` + `tests/test_work_command.py` (72 tests), with
  edits to `commands/default_cmdsets.py` and `typeclasses/characters.py`.
  Execution is **delayed** (`utils.delay`), which was not in this spec and is
  the component's one substantive addition — see the Rev 6 header for the five
  hazards it brings and how each is guarded, and Evennia Reference §11.27 for
  the cancellation pattern. The task table is `TEMPLE_TASKS` in the command
  module and carries flavour text per chore in addition to `copper`/`cooldown`.
  The place gate is `get_treasury().location == caller.location` with no second
  settings key: **moving the Treasury moves the faucet**, and "no Treasury
  configured", "Treasury elsewhere" and "unresolvable dbref" collapse into one
  sentence for the player, who is not the admin.
...
- **Status:** ✅ **CLOSED** (`2c75877`, 2026-08-03). Shipped as an extended
  `world/barter.py` + `tests/test_barter_currency.py` (40 tests), 296 green.
  Three deviations beyond the spec, all forced by the spec's own consequences:
  **(1)** `CmdPWOffer.func()` no longer delegates to the contrib — upstream
  returns early when no object resolves, so a coin-only offer is impossible
  through it, and it has no parameter to carry an amount; **(2)** `CmdPWDecline`
  is new and not in this spec — upstream's emptiness gate reads `list()`, which
  returns the two *item* lists, so allowing coin-only offers makes it refuse to
  decline a deal `status` is displaying; **(3)** `CmdPWStatus` is new and the
  globals patch grows from five swaps to seven — §6/E.1 assumed the rendering
  came free because barter owns the `status` key inside a trade, which is true
  of the *key* and not of the *class*.
...
- **Status:** ✅ **CLOSED** (`8d189f5`, 2026-08-03). 20 further tests, 316 green;
  in-game two-session protocol passed (happy path, `pay` mid-trade into a stale
  cancel, stale item, forced teardown). Two deviations. **(1)** The cancel
  wording is **two** messages, not this spec's "existing single message path": a
  player told *"an offered item is no longer available"* when it was the silver
  that vanished goes looking for the wrong thing. `_offered_currency_still_held()`
  exists and is called from both named sites, via a thin selector
  `_stale_offer_reason()` that owns the choice so the two call sites cannot drift
  apart. **(2)** Settlement is **net** while re-validation is **gross** — two
  different questions, deliberately given two different numbers. The guard asks
  whether each side still backs what it *promised* (net would pass a side that
  offered 50 holding 20 whenever the difference happens to be affordable); the
  transfer asks how little needs to move, because one `transfer_to` cannot
  half-settle the way two can. The messages are built from the gross figures,
  since the players agreed in gross and netting must not leak into the language.
  ⚠️ Recorded because it changed a docstring: the "unreachable" branch handling a
  failed `transfer_to` **fired** under mutation testing the moment the guard was
  weakened, and was the only thing preventing money from moving on a promise
  nobody could keep. Defence in depth, demonstrated rather than assumed.
...
- `add()` has exactly one caller in **production** code — `@economy mint` (C.2) —
  and a test proves it rejects every other source. Test code calls `add()`
  directly to fund wallets, and must: there is no other way to put money into a
  fixture. The invariant is about which *game* paths can create value, not about
  the literal number of call sites in the repository.
