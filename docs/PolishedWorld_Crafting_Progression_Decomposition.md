...
> **Rev 9 · 2026-07-11** — Doc hygiene (origin-trim): §13 active backlog items migrated to the consolidated `docs/BACKLOG.md` (single source of truth) and replaced with one pointer line; the two done-markers (`repair`→D.4, `CmdRepair` generalisation→G.2) kept as history. No code change; Stage 2 remains closed.
> **Rev 8 · 2026-07-11** — Component G (superior-tool scaling) **complete & in-game-verified** — **Stage 2 CLOSED**. G.1: `MongooseCraftRecipe._tool_modifier` gains a superior branch — a tool crafted at the critical tier (`quality > 100`, read live from the tool's own `db.quality`, `None`-guarded for uncrafted/admin tools) grants `+tool_bonus` (10); a plain present tool stays baseline 0, absent/broken stays penalty (−20). Tool recipes gain a `superior <key>` alias via a shared module-level `_apply_tool_quality()` (mirrors `_apply_garment_quality`, minus the condition table — tool start-condition stays prototype-driven 40/30). G.2: `CmdRepair._tool_modifier(caller, target)` generalised per target-type — reads `target.db.repair_tool_tag` (parallel to `repair_materials`): unset → needle default (garments unchanged), `""` → no tool (stone knife / bone needle set `repair_tool_tag=""`), so a carried needle no longer wrongly shifts a stone-knife repair; a *superior* repair tool grants +10 (`SUPERIOR_TOOL_BONUS`), the target is excluded from the tool search, and broken tools count as absent. **Ceiling reconcile:** §3 max-quality **110 → 111** (a superior tool's +10 with `skillcheck` never clamping `target`). Next: Stage 3 (Recipe Knowledge & Discovery), own decomposition.
...
1. **Waterskinens `>=125`-gren är död kod.** ✅ *Fixad i E.2 (Rev 6).* Efter A-flippen är ett *vanligt* verktyg baseline 0, så utan superior-verktyg är max quality **110** (skill-cap 100 → `crit_score = 10`). **Component G (Rev 8)** återinför den positiva modifieraren: ett *superior*-verktyg ger `+tool_bonus = 10`, och `skillcheck` klampar aldrig `target`, så med ett superior-verktyg blir max `target` = 110 → `crit_score = 11` → **max quality = 111**. (Den döda `>=125`-grenen antog ett +20-verktyg → 112; med det låsta +10 landar det verkliga taket på **111**.) Tröskeln 125 nåddes aldrig; band-helpern (`superior = quality > 100`) ersätter de råa trösklarna. **Avsiktlig edge:** en critical vid modifierad skill 0–9 har `crit_score = 0` → quality exakt 100 → bandas `serviceable`, inte `superior`. Det är capability-banding — bandet rapporterar föremålet (quality 100 = success-kapabilitet), inte tärningen; varje realistisk crafter (modifierad skill ≥ 10) får superior på crit ändå.
...
Ett superior verktyg är enda vägen över baseline — knyter ihop verktygs- och kvalitetstrådarna. **Klar & in-game-verifierad (Rev 8); Stage 2 stängd.**

### Task G.1 — Superior verktyg ger `+tool_bonus` ✅
- **Goal:** Ett superior-craftat verktyg (quality > 100) skalar craft-modifieraren positivt; vanligt verktyg = 0, trasigt/frånvarande = penalty.
- **Dependencies:** A.1 (flippen), C.1 (`Tool`), E.1 (`quality_band`/`SUPERIOR`), E.3-mönstret (`_apply_garment_quality`).
- **Implementation (shippat):** `_tool_modifier` läser verktygets EGNA `db.quality` (redan stämplad av `do_craft` på varje output *före* `_finalize_item`) via `quality_band`: superior → `self.tool_bonus` (låst **+10**); vanligt present → 0; frånvarande/brutet → `improvised_penalty`. **`None`-guard**: ett ocraftat/admin-spawnat verktyg (metall-`KNIFE`, ingen craft) har `db.quality = None` → kortsluts till baseline 0 före banding (annars `quality_band(None)` → TypeError). Brutet superior räknas frånvarande (penalty) via `_used_tool`:s `is_broken`-guard. Verktygsrecepten (`StoneKnifeRecipe`/`BoneNeedleRecipe`) fick `_finalize_item` som ENBART sätter superior-`aliaset` via modulnivå-funktionen `_apply_tool_quality(obj)` (speglar `_apply_garment_quality`, UTAN condition-tabell — verktygets start-condition är prototypdriven 40/30, D.5).
- **Testing (verifierat):** spawna verktyg, stämpla `db.quality = 110` → `_tool_modifier()` = **10**; `= 100` → **0**; ocraftat (quality None) → **0**; frånvarande → **−20**; brutet superior → **−20**; superior-alias sätts vid quality 105, ej vid 100.
- **Commit:** `feat(crafting): superior tools grant a positive modifier (ties quality to tools)`

### Task G.2 — `CmdRepair._tool_modifier`-generalisering + superior-bonus ✅
- **Goal:** Reparations-modifieraren väljer förväntat verktyg per target-typ (data-drivet) i stället för att alltid leta needle; superior repair-tool ger +10; brutet/frånvarande = penalty. Fixar §13-buggen (buren synål påverkade stenknivs-reparation).
- **Dependencies:** D.4 (`CmdRepair` `DurableObject`-gate + `repair_materials`), G.1.
- **Implementation (shippat):** `_tool_modifier(caller, target)` läser `target.db.repair_tool_tag` (parallellt med `repair_materials`): unset (`None`) → `DEFAULT_REPAIR_TOOL = "needle"` (plagg behöver ingen prototyp-ändring), `""` → inget verktyg (flat 0), `"<tag>"` → den `crafting_tool`-taggen. Superior buret verktyg → `SUPERIOR_TOOL_BONUS = 10`; vanligt → 0; frånvarande/brutet → `IMPROVISED_PENALTY`. `target` exkluderas från sökningen (aldrig sitt eget verktyg); brutna verktyg hoppas över (`is_broken`), konsekvent med craft-sidan. `STONE_KNIFE`/`BONE_NEEDLE`-prototyperna sätter `repair_tool_tag = ""` (spawner-verifierat att `""` lagras). Konstanten `NEEDLE_BONUS` (20, reserverad) → `SUPERIOR_TOOL_BONUS` (10).
- **Testing (verifierat):** plagg utan synål → **−20**; plagg + vanlig synål → **0**; plagg + superior synål → **10**; stenkniv + synål buren → `['', 0]` (synålen irrelevant — buggen fixad); stenkniv utan verktyg → **0**; plagg + bruten synål → **−20**.
- **Commit:** `feat(crafting): generalise CmdRepair tool modifier per target + superior bonus`

---

## 13. Backlog (utanför denna stages scope)

Stage 2 är stängd. De aktiva backlog-itemen har migrerats till den samlade
**`docs/BACKLOG.md`** (en sanningskälla) — se den för aktuell status, trigger och
kontext. Klarmarkeringarna behålls här som historik.
...
- **`CmdRepair._tool_modifier`-generalisering:** ✅ *Klar i G.2 (Rev 8).* Läser nu `target.db.repair_tool_tag` (unset → needle-default för plagg; `""` → inget verktyg för stenkniv/bennål), så en buren synål påverkar inte längre en stenknivs-reparation. Superior repair-tool ger +10.
- **Migrerat → `docs/BACKLOG.md`:** waterskin-`durability`→`condition`, jakt-oberoende needle-primitiv, metallverktyg + station/forge, fumble-extra-wear-tuning (`tool_wear_on_fumble`), individuering bortom "superior" (material-/maker's-mark-alias), samt superior-tool-longevity (ny G-deferral).
