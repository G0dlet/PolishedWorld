...
> **Rev 11 · 2026-07-26** — **Correction, no rebuild.** Rev 9's H.1 justification (1) — that keying a global `accept` would *break* — rested on Evennia Reference §11.20, which was itself wrong: it derived the tie-break from `cmdsethandler.update()` instead of the real runtime merge in `cmdhandler.get_and_merge_cmdsets()`. Corrected in Reference **Rev 15**: on a priority tie the **later-merged** cmdset wins. Barter's `CmdsetTrade` is added at trade start and therefore sits *above* `CharacterCmdSet` in the stack, so a same-keyed `accept` of ours would have lost **inside** a trade and worked **outside** it — an inversion, not the silent erasure of `CmdPWAccept` and its `_all_offers_in_hand` backstop that Rev 9 described. Nothing would have broken in either direction. Confirmed in game 2026-07-26 by a pair that has coexisted since Stage 2 and should have been broken under the old rule: our `CmdStatus` (`status`/`vitals`) and barter's (`status`/`offers`/`deal`). **The decision stands and no code moves** — extending `CmdLearn` to a third carrier is still right, because `learn` is the student's verb for scroll, book and teacher alike and a second verb would split one concept across two keys. Only the *reason* is replaced: UX, not mechanics. §13's H.1 block gains a correction bullet; the stale rationale in `commands/default_cmdsets.py`'s `CmdTeach` comment is fixed in the same sweep. Rev 9's text is left standing as the historical record it is — this entry supersedes its point (1), it does not rewrite it.

> **Rev 10 · 2026-07-26** — **Stage 3 CLOSED.** — **Stage 3 CLOSED.** Shipped Components **A–H**, in-game-verified on `feature/recipe-knowledge` (head `05e6df02`): the knowledge foundation (A), the dual gate (B), the `recipes` discovery surface (C), and **five knowledge channels** — profession-grants (D), reverse-engineering (E), scroll (F), perishable book (G) and live teaching (H). **Component I (thin world-loot scroll seed) was deferred in full to `docs/BACKLOG.md`, not built** — §14 named it a defer-candidate if world-content plumbing wasn't ready, and source-verification found `world/batch_cmds.ev` to be untouched Evennia boilerplate with zero build commands. Three reasons carried the decision: the failure mode it guards (a recipe unreachable because no online player holds its granting profession) **requires players**, and there are none; the valve is already operable manually in one `@py` line, so deferring costs automation rather than capability; and choosing a seed location/cadence/respawn policy against a world that does not exist would need redoing. The BACKLOG entry preserves the profession-coverage analysis (`linen shirt` and `leather boots` each hang on a single profession — the exposed pair — while `cloth` and `leather` have two grantors each), flags that this table is an artefact of the current 4-recipe slice and must be recomputed against the catalogue at implementation time, and sharpens the trigger: exposure is **highest at low player counts**, so the trigger is *before the first real player cohort*, not "when the world is large". A separate BACKLOG entry (new *Tooling & Process* section) logs the uncommitted `tests/` initiative honestly. §14 trimmed to a pointer. Next: merge to `main`, then Stage 4 (In-game Currency).
> **Rev 9 · 2026-07-26** — Component H (teaching — synkron personlig transfer med samtycke) complete & in-game-verified on `feature/recipe-knowledge` (`05e6df02`), en atomisk task: **H.1** (`CmdTeach`/`key="teach"` — `teach <recipe> to <player>` parkar ett samtyckes-offer på eleven och meddelar båda; eleven svarar med `learn <recipe> from <teacher>`, dvs. `CmdLearn` utökad till en **tredje bärare** i stället för en ny nyckel — `learn` är elevens verb för scroll, bok och lärare — och `_learn_from_teacher` körs FÖRE bärar-sökningen eftersom den senare är inventarie-scopad; guard-ordningen speglar `inscribe`/`scribe`: resolve → `requires_knowledge` → `_can_transmit` → target-validering → cooldown). **Tre avvikelser från §13:s ursprungsspec, alla källverifierade och motiverade:** **(1) accept-mekanismen** — `accept` avvisad, inte av stilskäl utan för att den *går sönder*: barters bara `accept` (alias `agree`) lever i `CmdsetTrade` med `priority = 0`, samma prioritet som vår `CharacterCmdSet`; Evennias `CmdSet._union` ger vid oavgjord prioritet segern till det **redan mergade** setet (`existing_commands = set(cmdset_a.commands)`) och `Command.__eq__` matchar på key+alias-intersektion, så en global `accept` hos oss hade **tyst raderat** `CmdPWAccept` — inklusive dess `_all_offers_in_hand`-backstop — under hela trade-sessionen. Barter använder dessutom själv ingen bar `accept` för sin *invite*-handshake, utan `trade <person> accept`, dvs. samma verb + argument. **(2) `ndb`, inte `db.pending_teach`** — ett pending offer är sessionstillstånd och får inte överleva en `@reload`, samma skäl som barters `ndb.tradehandler`; lagras på **eleven** (`student.ndb.pending_teach = (teacher, recipe_name, expires_at)`), som är den part som ska svara. **(3) cooldown vid OFFER, inte vid accept** — teaching är gratis, så det enda kommandot kan trycka på en ovillig spelare är det oombedda meddelandet, och cooldown-vid-accept rate-limitar exakt inget av det. **Invariant: `TEACH_COOLDOWN` (120) ≥ `TEACH_TIMEOUT` (60)**, vilket gör MVP-låset "en elev åt gången" *strukturellt* i stället för bokfört — ett gammalt offer har alltid förfallit innan ett nytt får skickas, så det finns aldrig mer än ett levande offer per lärare och ingen andra tillståndskälla att hålla synkad. Två designval till: känd-set-probe medvetet **utelämnad** vid offer (att fråga "kan du redan detta?" skulle göra `teach` till en vem-kan-vad-scanner, riktig underrättelse i en kunskapsekonomi — eleven får veta vid accept), och **full backstop-omvalidering vid accept** per barters `finish()`-lärdom: timeout, samma rum, lärare fortfarande inloggad, `_can_transmit` om, receptet fortfarande i registret och fortfarande `requires_knowledge`. Verifierat in-game inklusive regressionstest att ett levande offer **inte** skuggar scroll/bok-vägen (`learn cloth from <bok>` med utestående cloth-offer tar bok-vägen och lämnar offret orört). BACKLOG Rev 11 loggade teach-kanalens tuning-konstanter och flyttade *Teaching-som-amplifier* till sitt kanoniska hem. Status pointer advanced to Component I.
> **Rev 8 · 2026-07-25** — Component G (bok — förgänglig bulk-transfer) complete & in-game-verified on `feature/recipe-knowledge`, tre atomiska tasks: **G.1** (`typeclasses/books.py::Book(DurableObject, Object)` — `stamp(recipes)` äger identiteten och sätter **både** `db.recipes` och en riktig lagrad `key`, eftersom barter listar/matchar offers på `obj.key`; `get_display_desc` renderar flavour + recept-**namnen** + `condition_line()`; registry-fri, namnen läses ur `db.recipes`; BOOK-prototyp utan bakade recept/condition, start-condition sätts av `scribe`); **G.2** (`CmdScribe`/`key="scribe"` — `scribe <recipe>, <recipe>[, ...]` med valfritt `book of `-prefix; validerar HELA listan + de-duplicerar före något förbrukas, per-recept `_can_transmit` + flat bok-tröskel `SCRIBE_MIN_CRAFT = 50` läst som permanent `.current` medan condition-rullen läser effektiv `.value`, `SCRIBE_MATERIAL_TAGS = cloth×2 + twine` gather-then-consume atomärt, `SCRIBE_COOLDOWN = 120` före material, spawn först → delete → `stamp()` → condition; misslyckad scribe ger ändå en bok med låg condition, speglande craftets `consume_policy="raw"`); **G.3** (`CmdLearn` utökad till böcker — `learn <recipe> from <book>`, parse-split på text till vänster om ` from `, dispatch på `db.recipes is not None`, scroll-vägen F.2/F.3 oförändrad; `BOOK_WEAR_PER_STUDY = 20` → 2–5+ läsare beroende på bindningskvalitet, alltid fler än scrollens 1; **complete-then-crumble låst**: studien som når condition ≤0 slutförs, eleven får receptet, sedan `book.delete()`; redan-känt eller ej-i-boken nöter inget). Tre doc-rättelser: §12 G.1:s "lista ej namnen" **reverserad** (val B — namnen listas; barter matchar på `key`, så en oläsbar bok vore ohandelbar; ingredienser kräver fortfarande studie), §12 G.2:s materiallista rättad (parchment×N + leather-omslag → **cloth×2 + twine**; parchment BLOCKED, leather DECISION/obyggt), och §12 G.2:s "speglar E.3-mönstret" förtydligad till en **cross-doc**-referens (E.3 lever i *Crafting Progression*-decompen; Component E i denna doc har bara E.1/E.2). BACKLOG Rev 10 loggade bok-kanalens tuning-konstanter, utökade parchment-posten med bokomslaget, flyttade in bok-repair i sitt kanoniska hem, och loggade att scribes band→condition medvetet går förbi `crafting_quality.quality_band`. Status pointer advanced to Component H.
> **Rev 7 · 2026-07-18** — Component F (scroll — the first *written* knowledge channel) complete & in-game-verified on `feature/recipe-knowledge`, four atomic tasks: **F.1** (`world/knowledge.py::_can_transmit` shared mastery gate + `CmdInscribe`/`key="inscribe"` — writes a one-use scroll for a mastered recipe, stamps `obj.db.recipe`, consumes a bolt of cloth; writing-material (a) locked to **cloth** MVP, parchment deferred → BACKLOG; skill read (b) locked to permanent `.current`, mirroring `improve_skill_on_use`; no roll, pure threshold); **F.2** (`CmdLearn`/`key="learn"` — study a scroll to learn its recipe permanently, scroll consumed; already-known → kind no-op, scroll kept; new common-guard keeps the known-set advanced-only); **F.3** (readable scrolls — `render_recipe_detail(cls)` extracted to `world/knowledge.py`, shared byte-identically by `recipes <name>` and a new `Scroll` typeclass whose `get_display_desc` shows recipe detail on `look`; `render_recipe_detail_by_name` keeps registry access consolidated so the typeclass isn't a fifth `_RECIPE_CLASSES` consumer); **F.4** (scroll identity moved onto the `Scroll` typeclass — `stamp(recipe_name)` sets recipe + a real searchable `key`, since barter lists/matches offers by `obj.key`; flavour + detail render live from the stamp, so every creation path is self-describing; `inscribe` slimmed to `scroll.stamp(...)`). BACKLOG Rev 8 logged the `INSCRIBE_COOLDOWN` tuning, parchment (BLOCKED) and blank-scroll deferrals, extended the `_RECIPE_CLASSES`-coupling entry to four consumers, and noted the shared prettify renderer. Status pointer advanced to Component G.
> **Rev 6 · 2026-07-13** — Component E (recipe-stamp + reverse-engineering, the first *item* knowledge channel) complete & in-game-verified on `feature/recipe-knowledge`, two atomic tasks: **E.1** (`world/crafting_base.py::do_craft` — one line, `obj.db.recipe = self.name` stamped after `crafted_by` in the spawn loop; only player-crafted output carries it, spawned/loot/admin items leave `db.recipe` None so reverse-eng bites only on crafted goods; also seeds the BACKLOG maker's-mark) and **E.2** (`commands/crafting_commands.py::CmdDisassemble`, `key="disassemble"`/alias `salvage` — sacrifices a stamped item for a Craft-roll to learn its recipe, destroyed win or lose; guards refuse a None/common/already-known target *before* any destruction; difficulty scales via `modifier=-min_skill`; success routes through `attempt_skill_improvement("craft", outcome)` as the fifth check-site). Two architecture decisions locked: E.2 resolves the stamped name by **exact** `_RECIPE_CLASSES.get(name)` + None-guard (the stamp is the canonical `self.name`; a removed recipe → unlearnable, no destroy), not the fuzzy `_resolve_recipe`; and `DISASSEMBLE_COOLDOWN = 300` as a conservative named constant (tuning deferred → BACKLOG). The doc's §10 `_RECIPE_CLASSES[name].min_skill` note was refined in code to the exact-get + `getattr(...,0) or 0` idiom. Wired after `CmdRecipes`. BACKLOG Rev 7 logged the cooldown-tuning deferral and extended the `_RECIPE_CLASSES`-coupling entry (E.2 is a second consumer). Status pointer advanced to Component F.
...
**Status:** **CLOSED.** Components A–H complete & in-game-verified on `feature/recipe-knowledge` (head `05e6df02`); Component I deferred in full → `docs/BACKLOG.md`. Remaining: merge to `main`, then Stage 4 (In-game Currency).
...
### Task F.3 — Läsbara scrolls (`look` visar receptets detalj) *(polish; session-tillägg)*
- **Goal:** Läs en scroll utan att förbruka den — `look <scroll>` visar receptets Needs/Tool/Skill/Output, precis som `recipes <name>`.
- **Dependencies:** F.1 (scroll-prototyp + `db.recipe`-stämpel), C.2 (renderaren).
- **Implementation:** Extrahera C.2:s renderingsblock till `world/knowledge.py::render_recipe_detail(cls)` (ren funktion, ingen visibility-gate; byte-identisk med C.2:s tidigare output) + `render_recipe_detail_by_name(name)` som håller `_RECIPE_CLASSES`-uppslaget inne i modulen. `typeclasses/scroll.py::Scroll(Object)` med `get_display_desc` som hänger detaljblocket på `look` (DurableObject-precedenten). Byt `SCROLL`-prototypens typeclass → `typeclasses.scroll.Scroll`. C.2 → `caller.msg(render_recipe_detail(cls))`; `Counter`-importen flyttar med. Ingen visibility-gate: innehav *är* gaten (håller du scrollen kan du redan `learn` gratis). Val: **f2** (namn + full detalj), **g1** (typeclass + `get_display_desc`), **h** (renderare i `world/knowledge.py`).
- **Testing:** `@py` handstämpla scroll; `look scroll` → detaljblock, scroll kvar; blank scroll → bara base-desc, ingen krasch; `recipes cloth` → oförändrad.
- **Commit:** `feat(knowledge): render recipe detail on look for inscribed scrolls`

### Task F.4 — Scroll-identitet på typeclassen *(polish; session-tillägg)*
- **Goal:** Varje scroll är självbeskrivande (namn + flavour) från sin stämpel, oavsett skapande-väg — inte bara de som gått via `inscribe`.
- **Dependencies:** F.1, F.3 (`Scroll`-typeclass).
- **Implementation:** `Scroll.stamp(recipe_name)` äger identiteten: `db.recipe` + en **riktig** `key` (`"scroll of <recipe>"`). Nyckeln måste vara lagrad (inte dynamisk `get_display_name`) eftersom barter listar/matchar offers på `obj.key` (`world/barter.py` + contribens `list()`). `get_display_desc` genererar flavour (blank vs inskriven) + detalj live från stämpeln, inte från bakad `db.desc`. `inscribe` slimmas till `scroll.stamp(recipe_name)` (inget spawn-ändring — prototypen är redan `Scroll`). Val: **j2** (identitet på typeclassen via `stamp()`); j3 (dynamisk `get_display_name`) förkastad — barter läser `key`.
- **Testing:** `@py` `spawn("scroll")[0].stamp("cloth")` → `key == "scroll of cloth"`; två stämplade → distinkta i `inv`; blank → "waiting to be inscribed"; `inscribe cloth` end-to-end (cloth i hand) → "a scroll of cloth".
- **Commit:** `feat(knowledge): move scroll identity onto the Scroll typeclass`
...
- **Implementation:** `typeclasses/books.py::Book(DurableObject, Object)` — ärver `condition`/`apply_wear`/`is_broken`/`get_display_desc` (condition-rad på `look` gratis). `db.recipes` = lista recept-namn. `get_display_desc` utökas: condition-rad + recept-**namnen** ("It holds recipes for: cloth, leather"). **Rev 8 reverserar Rev 7:s "lista ej namnen"** (val B, låst i G-chatten): `stamp()` sätter en riktig lagrad `key` eftersom barter listar/matchar offers på `obj.key` — en bok som inte röjer sitt innehåll vore ohandelbar. Mystiken bevaras ändå: `look` visar *vilka* recept, aldrig deras ingredienser (det kräver studie). Ingen repair (MVP — bevarar sinken; bok-repair → BACKLOG). Start-condition sätts av `scribe` (G.2), ej hårdkodat här.
...
- **Implementation:** `CmdScribe` (`key="scribe"`, `scribe <recipe>, <recipe>[, ...]`; valfritt `book of `-prefix accepteras, `stamp()` äger prefixet i nyckeln): parsa komma-listan; **kräv `_can_transmit` för VARJE listat recept** (du kan bara skriva ned det du bemästrat) + common-guard per recept; HELA listan valideras och de-dupliceras **före** något förbrukas, så ett felstavat namn kostar inget material. Bok-tröskel **låst: flat `SCRIBE_MIN_CRAFT = 50`** ("professional", Legend p.72–73), läst som permanent `.current` (som `_can_transmit`) — en tillfällig buff ska inte ge auktoritet att författa en bestående bok, medan condition-**rullen** läser effektiv `.value` (som `do_craft._skill_value`), så en buff kan lyfta bindningens *kvalitet*. Material **låst: `SCRIBE_MATERIAL_TAGS = ["cloth", "cloth", "twine"]`** (två bal tyg till sidor, en längd snöre till bindning) — parchment/leather-omslag deferrat (parchment BLOCKED, leather DECISION/obyggt) → BACKLOG; alla material kontrolleras innan NÅGOT förbrukas (atomärt under reaktorn). `SCRIBE_COOLDOWN = 120` kollas efter de ofarliga guarderna men **före** material. Ordning: spawn `Book` → delete material → `stamp(recipes)` → sätt `condition` → `move_to`. Start-condition ur **`SCRIBE_CONDITION_BY_TIER = {critical: 100, success: 80, failure: 50, fumble: 25}`**, `+ crit_score` på kritisk (enda tiern som når pristine-och-över); värdena mappar avsiktligt på `DurableObject`s färgband (grön >66 / gul 33–66 / röd <33) så `look` läser bindningskvaliteten direkt. En **misslyckad scribe ger ändå en bok** med låg condition (material förbrukat) — speglar craftets `consume_policy="raw"`, inte total förlust. *Not: Rev 7:s "speglar E.3-mönstret" är en **cross-doc**-referens — E.3 (`_apply_garment_quality`, band→condition) lever i Crafting Progression-decompen; Component E i denna doc har bara E.1/E.2. Scribe mappar tier→condition direkt i stället för husets tier→quality→`quality_band()`→condition, eftersom kommandot aldrig beräknar eller stämplar ett `quality`-tal; avvikelsen loggad → BACKLOG.*
...
- **Implementation:** Utöka `CmdLearn` att ta böcker: `learn <recipe> from <book>` — validera att receptet finns i `book.db.recipes`; `learn_recipe(name)` (`False` = redan känt → meddelande, **nöt inte** boken); annars success + `book.apply_wear(BOOK_WEAR_PER_STUDY)`. **Mikro-val LÅST (G-chatten): slutför-då-vittra.** Studien som tar condition ≤0 *slutförs* — eleven får receptet — och först därefter `book.delete()`; alternativet "bruten bok lingrar" förkastades, då det lämnar husk-skräp och grumlar sinken. **`BOOK_WEAR_PER_STUDY = 20`** (låst): flat kostnad per lektion, så start-conditionen (scribes rull) avgör antalet läsare — fumble 2, failure 3, success 4, kritisk 5+ — alltid fler än scrollens ende läsare, skalande med bindningskvalitet. Parse: bok-formen känns igen på text till vänster om ` from `; dispatch på `db.recipes is not None`, så scrollvägen (F.2/F.3) är orörd. Redan-känt recept, recept som ej finns i boken, eller ett borttaget/omdöpt recept → meddelande, **nöt inte**.
...
**Status pointer:** Component G complete (G.1–G.3). **Component H next.**
...
- **Implementation (levererad):** `CmdTeach` (`key="teach"`, `teach <recipe> to <player>`, split på **sista** ` to `). Guards i `inscribe`/`scribe`-ordning: `_resolve_recipe` → `requires_knowledge` → `_can_transmit(caller, name)` (känner + `min_skill`; Teaching-*skill* EJ krävd) → `caller.search` (som defaultar till `location.contents + self.contents` och därmed *implicit* ger samma-rum-kravet) → inte sig själv → `student.has_account and hasattr(student, "learn_recipe")` (en spelad Character; avvisar objekt, NPC:er och opuppetade kroppar med ett meddelande) → cooldown. Offret parkas som `student.ndb.pending_teach = (teacher, recipe_name, expires_at)` — **`ndb`, inte `db`**: sessionstillstånd som inte får överleva en `@reload` (barters `ndb.tradehandler`-skäl) — och på **eleven**, som är den som ska svara. Cooldownen sätts när offret **skickas**, inte vid accept: det oombedda meddelandet är den enda ytan `teach` kan pressa på en ovillig spelare. **Ingen** probe av elevens known-set vid offer (skulle göra kommandot till en vem-kan-vad-scanner).
  **Accept = `CmdLearn` utökad, ingen ny nyckel.** Eleven svarar `learn <recipe> from <teacher>` (eller `learn from <teacher>` när bara ett offer ligger ute). `_learn_from_teacher` körs **före** bärar-sökningen — den är inventarie-scopad och skulle annars felmeddela om en lärare som står i rummet — och identifierar läraren med `caller.search(target_name, candidates=[teacher], quiet=True)`, så en icke-träff faller tyst igenom till scroll/bok-vägen. Backstop-omvalidering vid accept (barter-`finish()`-lärdomen): timeout, `teacher.location == caller.location`, `teacher.has_account`, `_can_transmit(teacher, name)` om, exakt registry-get + `requires_knowledge`. Offret retireras **före** `learn_recipe`, så ett andra accept blir en no-op i stället för en omkörning. Gratis (inget material, ingen roll).
- **Rättelse (Rev 11) — motiveringen till att INTE nyckla `accept` var mekaniskt fel.** Rev 9 hävdade att en global `accept` i `CharacterCmdSet` skulle ha *raderat* barters `CmdPWAccept` under hela trade-sessionen, med stöd i Evennia Reference §11.20. §11.20 var själv fel — härledd ur `cmdsethandler.update()` i stället för runtime-mergen i `cmdhandler.get_and_merge_cmdsets()` — och är rättad i **Reference Rev 15**: vid prioritetslikhet vinner det **senare mergade** setet. `CmdsetTrade` läggs till vid trade-start och ligger alltså ovanför `CharacterCmdSet` i stacken, så barters `accept` hade vunnit *inuti* trade och vår hade fungerat *utanför*. En inversion, inte en radering; ingenting hade gått sönder åt något håll. **Beslutet står oförändrat, men på UX-grund:** `learn` är elevens verb för scroll, bok och lärare, och en andra nyckel hade splittrat ett begrepp på två verb. Ingen kodändring följer — endast `commands/default_cmdsets.py`:s `CmdTeach`-kommentar är uppdaterad.
- **Låsta konstanter:** `TEACH_TIMEOUT = 60`, `TEACH_COOLDOWN = 120`. **Invariant `TEACH_COOLDOWN >= TEACH_TIMEOUT`** — det är detta som gör MVP-låset *en elev åt gången* strukturellt: ett gammalt offer har alltid förfallit innan ett nytt får skickas, så ingen back-referens på lärarsidan behövs. Bevara relationen vid tuning (→ `docs/BACKLOG.md`).
- **Testing (körd):** Tvåsessionstest (**inte** `@ipuppet` — det byter session, så båda parter kan inte vara inne samtidigt). Guard-batteri utan att bränna cooldown; happy path med `pending_teach`-inspektion före/efter; redan-känt-vägen; lärare lämnar rummet före accept → *"That lesson has lapsed."* och known-set orört; fel recept namngivet → *"…offered to teach you cloth, not leather."*; samt tre **icke-skuggnings**-regressioner: med ett levande cloth-offer tar `learn leather from <bok>`, `learn cloth from <bok>` och `learn from <scroll>` alla bärar-vägen och lämnar offret orört. Ett offer kan hand-stämplas för deterministiska tester: `@py str(me.search("Bob").nattributes.add("pending_teach", (me, "cloth", 9999999999)))`.
...
### Task I.1 — Seeda scrolls i världen  ⟶ **DEFERRAD, EJ BYGGD**

Deferr-klausulen i den ursprungliga specen utlöstes vid Stage 3:s close-out: världs-content-plumbing finns inte (`world/batch_cmds.ev` är orörd Evennia-boilerplate, noll build-kommandon), felläget kräver spelare som ännu inte finns, och ventilen är redan manuellt körbar i en `@py`-rad. Stage 3 stängde därför på **A–H**, med profession-grant, reverse-engineering, scroll, bok och teach som aktiva kanaler.

Hela posten — inklusive professions-täckningsanalysen, de bevarade designbesluten (`Scroll.stamp()`-disciplin, idempotens, engångs kontra respawn) och den skärpta triggern (*före första riktiga spelarkohorten*, eftersom exponeringen är störst vid **få** spelare) — bor nu i sitt kanoniska hem. → se `docs/BACKLOG.md`, *Crafting & Tools* → "World-loot scroll seed".
...
- **Teaching-skill som amplifier** — flyttad till sitt kanoniska hem, `docs/BACKLOG.md` (*Crafting & Tools*), vid Component H:s close-out. → se BACKLOG.md.
