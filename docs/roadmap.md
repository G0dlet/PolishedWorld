...
> **Rev 18 · 2026-08-14** — Stage 6's implementation note **answered**. It had said *"`evennia.contrib.grid.xyzgrid` is the likely base (verify at source)"* since Rev 2; the verification is now done and written up as `PolishedWorld_Evennia_Reference.md` §14 (read against Evennia `main`, diffed against the pinned `v6.1.0` tag, identical). The note is replaced with the working shape — **xyzgrid for authored structure, wilderness for resource areas, separate coordinate spaces, no built-in bridge** — and the stage gains four **Open questions**, plus one matching `[OPEN]` decision-log entry. The distinction this Rev is careful about: source reading produced *candidates*, not decisions. Writing them in as settled would be a banner rather than a fix. Two of the four questions touch code already shipped — `ResourceNode` and tag-based creature spawning both assume a room that survives being left, which is precisely what a recycled wilderness shell does not do — so they are logged where they will be seen before the stage starts, not left in a chat log. No sequencing changes; combat-first stands.
> **Rev 17 · 2026-08-12** — one decision-log entry **added**: **[OPEN] no alpha milestone is defined**. Nothing in the plan changes; what changes is that a gap acquires a home. Three items already condition themselves on a milestone this document never defines — character creation (*"if a launch milestone gets scheduled"*), recipe mastery (*"once the recipe catalogue exists"*) and the GameGold trigger (*"a player-count milestone"*) — so the milestone is load-bearing for sequencing while existing nowhere as a decision. Deliberately **not** a numbered stage: Rev 16 declined to number recipe mastery because its size cannot be judged before the recipe catalogue exists, and a Stage 4.75 would inherit exactly that problem, with the added cost that stage numbers are cited by name in `docs/BACKLOG.md` triggers and production comments — one invented on a guess is expensive to unwind. It is recorded at roadmap altitude rather than in `BACKLOG.md` for the reason Rev 16 gave: that file's scope is deferrals smaller than a stage, and this is sequencing. The entry also corrects a claim this document has never made but which the *absence* of `docs/crafting/` from it invites — that the catalogue must be authored from nothing. It must not: a full *Arms of Legend* dependency decomposition is already on `main`; what it lacks is the difficulty axis. The **[OPEN] Death-consequence policy** entry gains a **timing** clause from the same discussion — it is a player contract, and contracts are agreed before the cohort arrives, not during Stage 5. **Trigger to revisit: Stage 4.5 close-out.**
> **Rev 16 · 2026-08-11** — one parallel-backlog item **added**: **recipe mastery & craft disciplines**. It sits here rather than in `BACKLOG.md` because that file's scope is explicitly *deferrals smaller than a stage*, and this is stage-sized. It is **unnumbered** because its size cannot be judged until the recipe catalogue exists — the same content trigger that blocks Stage 4.5's Component E. The decision log records the ordering constraint, which is the part worth reading twice: switching `_can_transmit` from `min_skill` to a per-recipe mastery threshold leaves `min_skill` single-purpose, relieving one of the three readers behind E's dual-semantics blocker, so **mastery lands before E, not after** — though both wait on the same catalogue, so neither is scheduled. Also records why `PROFESSIONS` must not double as the display category: it is a deliberately *overlapping* starting-knowledge bundle (`cloth` sits under both weaver and generalist, by design, to force interdependence), while a category heading is one-to-many. Reusing it would be the same one-field-two-jobs shape that produced the E blocker in the first place.
...
**Implementation note:** source-verified 2026-08-14 — see `PolishedWorld_Evennia_Reference.md` §14 (read against Evennia `main` and diffed against the pinned `v6.1.0` tag; identical). Working shape is **two contribs side by side**: `evennia.contrib.grid.xyzgrid` for authored persistent structure (roads, villages) and `evennia.contrib.grid.wilderness` for large homogeneous resource areas (forest, mountain). They have separate coordinate spaces and **no built-in bridge** — grid↔wilderness transitions are project code at trailhead rooms. The shape is not yet a locked decision; see the open questions below. **Procedural generation is the long-term vision — initial scope is a hand-built frontier with scaling hooks**, not a generator.
...
**Open questions (all unlocked — source reading produced candidates, not decisions):**
1. **Shared room base.** Neither `XYZRoom` nor `WildernessRoom` inherits `ExtendedRoom`, so weather, seasonal descs and room states don't come along. Candidate is a mixin consumed by both sides; the grid side composes for free, the wilderness side needs the *behaviour* re-implemented in memory because its *storage* (Tags/Attributes on a recycled shell) leaks between coordinates (Reference §14.5, §14.7).
2. **Resources: coordinate-data vs objects-in-rooms.** The wilderness contrib's per-step lookup is O(total objects in the wilderness) (Reference §14.6), which argues for data materialised into an object only on harvest. That conflicts with how `ResourceNode` works today, and with tag-based creature spawning — both assume a room that survives being left.
3. **Building workflow for grid areas.** `xyzgrid spawn` deletes rooms off-map and overwrites prototype-defined attributes (Reference §14.2), so grid areas are authored in versioned map modules rather than in-game. Habit change, and the command is destructive.
4. **Sub-sequencing within the stage.** Grid-first is the candidate — smaller, lower risk, and it yields in-game minimap and pathfinding immediately (Reference §14.8).
...
- **Recipe mastery & craft disciplines** — a per-recipe completed-craft counter (`RecipeMasteryHandler` as a `@lazy_property` on Character, mirroring `world/skill_xp.py`; no backfill), written from exactly one production call-site: `MongooseCraftRecipe.do_craft`, inside the `_should_produce` branch, so successes and criticals count and fumbles do not. Mastery is a **roll modifier, never a gate** — +5 per 5 crafts, hard cap +20, which exactly cancels `improvised_penalty` and lifts the quality ceiling 111 → ~113 — the same stance `_tool_modifier` already takes. It closes two measured gaps: `_can_transmit` reads *global* Craft today, so a smith who has never made a waterskin may teach it; and every recipe rolls the same `craft` trait, so specialisation has no mechanical existence at all. A new one-per-recipe `discipline` class attribute groups the catalogue for display (`recipes` gains discipline-summary and discipline-list modes) and carries cosmetic titles ("Journeyman Weaver"); mechanical cross-recipe aggregation is **rejected** — it would reintroduce the generic-competence gap the counter exists to fix. `discipline` is deliberately *not* `PROFESSIONS`. Grinding is self-limiting: common recipes cannot be taught, so mastering twine is worth nothing. *(Consider elevating to a numbered stage once the recipe catalogue exists.)*
...
- **[RESOLVED] Craft specialisation model → per-recipe mastery, not split Craft skills (2026-08-11).** Legend's own answer is `Craft (Weaver)`-style specialisation skills, and it would reuse Stage 4.5's machinery untouched — but it needs a DB-stored trait migration and still cannot tell a waterskin from twine inside one trade. Per-recipe mastery serves both goals (a real teaching gate, and specialisation that pays) more cheaply. **Ordering constraint:** switching `_can_transmit` from `min_skill` to a mastery threshold leaves `min_skill` single-purpose, so mastery lands **before** Stage 4.5 Component E, not after; both wait on the same recipe-catalogue trigger. The `discipline` attribute is the forward seam if Legend-style specialisation skills are ever adopted — `skill_name` would be derived from it rather than hardcoded to `"craft"`.
- **[OPEN] No alpha milestone is defined — and three items are already waiting on one.** `Legend character creation` defers to *"if a launch milestone gets scheduled"*; `recipe mastery & craft disciplines` to *"once the recipe catalogue exists"*; **[OPEN] GameGold trigger** to *"a player-count milestone."* None of the three can be scheduled, because the thing they defer to is not a decision anyone owns. **What a closed alpha would minimally need, in dependency order:** a recipe catalogue laddering across the skill scale — which also unblocks Stage 4.5 Component E *and* recipe mastery, so one piece of content clears three items → per-recipe mastery → Component E → Legend character creation (without it every character is identical, so there is no variation to balance against and nothing for the stat-derived mechanics to vary) → **death-consequence policy settled** → the minimum of *Launch readiness* (admin/anti-grief, faucet-vs-sink view). Combat is **not** on this list: the gather → craft → trade → survive loop is closed without it, and pillar 1 is testable without stakes. **The catalogue is further along than this document implies:** `docs/crafting/` carries a complete *Arms of Legend* dependency decomposition, so the remaining work is not authoring one but **laddering `min_skill` across its `[DATA]`-buildable subset** — `min_skill` appears zero times in that decomposition, and 152 nodes are `[BLOCKED]` on unbuilt systems against 110 `[DATA]`. **Open, and settled by judgement rather than by measurement:** whether that subset spans enough of the scale to ladder at all — if it tops out below the intended ceiling the gate reproduces today's failure one rung higher. Detail and trigger live in `docs/BACKLOG.md` (*Crafting & Tools*, BLOCKED). **Not scheduled, not numbered** — recorded so it gets scheduled deliberately at the next stage-planning point rather than assembled ad hoc.
- **[OPEN] Stage 6 world architecture — grid + wilderness split (candidate, not decided).** Source verification on 2026-08-14 closed the *"verify at source"* note that had sat in the Stage 6 entry since Rev 2, and the reading is written up in `PolishedWorld_Evennia_Reference.md` §14. What it produced is a **working shape, not a decision**: xyzgrid for authored structure, wilderness for resource areas, project code for the transition. Four questions stay open and are listed under the Stage 6 entry — shared room base, resources as coordinate-data vs objects, the grid building workflow, and grid-first sub-sequencing. Two of them touch systems already shipped: `ResourceNode` and tag-based creature spawning both assume a room that persists when you leave it, which is exactly what the wilderness contrib does not provide. Nothing is committed here; the stage still needs its own design session before decomposition.
...
- **[OPEN] Death-consequence policy** — Stage 0 built the *mechanic* (HP 0 → death → player-corpse), but the *consequence* is undecided: permadeath or not, item loss on death, corpse-retrieval runs? It's simultaneously a stake (combat) and a sink (economy), and it interacts with coin-as-objects (Stage 4) — decide it deliberately, not implicitly in a commit. **Timing:** it is a player contract, not only a mechanic — settle it *before* the first real test cohort, not during Stage 5. Retro-fitting item loss onto players who started without it is a different act from shipping with it.
