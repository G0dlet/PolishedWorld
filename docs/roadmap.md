...
- **Stage 2 — Crafting progression & tools** (partial) — Components A (tool-modifier flip: present tool = baseline 0, absent = penalty), B (shared `condition` durability axis via `typeclasses/durable.py::DurableObject`, inherited by clothing and tools), C (tool bootstrap: `Tool(DurableObject, Object)`, `stone`/`stick` gatherables + nodes, `StoneKnifeRecipe` & `BoneNeedleRecipe`), and D (wear sink: D.1 per-use wear / D.2 broken = absent, not deleted / D.3 condition on `look` / D.4 repair convergence to tools + garments with data-driven materials / D.5 bootstrap start-condition 40/30, prototype-override verified) complete & in-game-verified. Zero-to-tool loop playtested; the source→sink→repair loop now closes. Remaining Stage 2: E (quality) → F (skill-gate) → G (superior tools). Canonical doc: `PolishedWorld_Crafting_Progression_Decomposition.md` (Rev 5).
