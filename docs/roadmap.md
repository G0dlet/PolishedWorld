...
### 🔄 In progress on `feature/recipe-knowledge`
- **Stage 3 — Recipe knowledge & discovery** (foundation laid) — Component A complete & in-game-verified: A.1 (per-character tag-based known-recipe set on `Character` — `KNOWN_RECIPE_CATEGORY` + `knows_recipe`/`learn_recipe`/`known_recipes`) and A.2 (`requires_knowledge` flag on `MongooseCraftRecipe`; `True` on cloth, leather, linen shirt, leather boots — the survival/tool four inherit `False`). Storage + declaration only; the gate that reads them is Component B (next). Canonical doc: `PolishedWorld_Recipe_Knowledge_Decomposition.md` (Rev 2). *(Stage 2 — Crafting progression & tools — is complete & merged to `main`; see the Rev 8 changelog and the Stage 2 detail below.)*
