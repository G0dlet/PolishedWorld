...
- **Stage 2 — Crafting progression & tools** (partial) — Components A (tool-modifier flip: present tool = baseline 0, absent = penalty), B (shared `condition` durability axis via `typeclasses/durable.py::DurableObject`, inherited by clothing and tools), C (tool bootstrap: `Tool(DurableObject, Object)`, `stone`/`stick` gatherables + nodes, `StoneKnifeRecipe` & `BoneNeedleRecipe`), D (wear sink: D.1 per-use wear / D.2 broken = absent, not deleted / D.3 condition on `look` / D.4 repair convergence to tools + garments with data-driven materials / D.5 bootstrap start-condition 40/30, prototype-override verified), E (quality → capability: `world/crafting_quality.py` band helper, waterskin/garment stats scaled by craft tier, `superior <key>` alias), and F (skill-gate: `min_skill` floor enforced in `pre_craft` before consume, reads effective skill; only `leather boots` gated at 30, all else ungated), and G (superior-tool scaling: a superior crafted tool grants +10 on the craft check + a `superior <key>` alias; `CmdRepair` tool modifier generalised per target via `repair_tool_tag`) complete & in-game-verified. Zero-to-tool loop playtested; the source→sink→repair loop closes and superior tools now measurably improve output (max craft quality 111). **Stage 2 COMPLETE.** Canonical doc: `PolishedWorld_Crafting_Progression_Decomposition.md` (Rev 8).
