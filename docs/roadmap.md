...
- **Stage 2 — Crafting progression & tools** (partial) — Components A (tool-modifier flip: present tool = baseline 0, absent = penalty), B (shared `condition` durability axis via `typeclasses/durable.py::DurableObject`, inherited by clothing and tools), and C (tool bootstrap: `Tool(DurableObject, Object)`, `stone`/`stick` gatherables + nodes, `StoneKnifeRecipe` & `BoneNeedleRecipe`) complete & in-game-verified. Component D (wear sink) underway: D.1 (per-use wear) / D.2 (broken = absent, not deleted) / D.3 (condition on `look`) done & verified; D.4 (repair convergence) + D.5 (bootstrap start-condition) remain. Zero-to-tool loop playtested; the source→sink loop now closes. Remaining: finish D → E (quality) → F (skill-gate) → G (superior tools). Canonical doc: `PolishedWorld_Crafting_Progression_Decomposition.md` (Rev 4).
