...
> **Rev 9 · 2026-09-09** — index `PolishedWorld_Timed_Actions_Decomposition.md` (Epic A), and **renumber the entry that added it, which shipped as a second `Rev 8` and below the first.** Three failures in one commit, and they are one failure at three altitudes. **The index gap:** third decomposition to reach `main` unlisted, and the second in a row where it was the file the adding session itself created — Rev 6 introduced the directory-vs-list loop, Rev 7 recorded that the loop was not run against the file Rev 7 was adding, and this records it again. The pattern is now specific enough to name: **the loop is run at session start, against the tree as it was, and the file that session creates is never re-checked.** The check belongs to the commit that adds a document, not to session start. **The duplicate number:** the entry was written by anchoring on `Rev 7` — the revision this changelog *discusses* — without reading the top of the list, where **Rev 8 · 2026-08-12** already stood. Two `Rev 8`s break rev-state verification, and every handoff in this project verifies rev-state by reading the first `Rev` header. **The ordering:** anchoring below `Rev 7` also placed the entry *under* an older revision, in a changelog whose only structural rule is that it descends. ⚠️ **All three are one mistake:** reading what was expected to be there instead of what is there. Rev 8's own lesson was that reading the list and reading the directory are different acts; this adds that reading a remembered revision and reading the current one are too. Nothing else about the index changes.
...
| Timed actions decomposition (Epic A) | tactical (feature tasks) | `docs/PolishedWorld_Timed_Actions_Decomposition.md` |
