...
> **Rev 8 · 2026-09-09** — index `PolishedWorld_Timed_Actions_Decomposition.md` (Epic A). **This is the third time a decomposition has reached `main` unindexed, and the second time in a row it happened to the file added by the very session that had the check in front of it.** Rev 6 introduced the directory-vs-list loop; Rev 7 recorded that the loop was not run against the file Rev 7 itself was adding; this Rev records the same thing again. The pattern is now specific enough to name: **the loop is run at the start of a session, against the tree as it was, and the file that session creates is never re-checked.** The check does not belong to session start — it belongs to the commit that adds a document, in the same breath as writing it. Nothing else about the index changes.
